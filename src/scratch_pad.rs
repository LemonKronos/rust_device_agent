...
impl PayloadMaker {
    ...
    pub fn process_batch_mock_up(&mut self, batch: &Vec<ScheduledTask>, force_full_scan: bool) -> Json {
        let task_map: FxHashMap<TaskID, ScheduledTask> = batch
            .iter()
            .map(|task| (task.id.clone(), task.clone()))
            .collect();

        // Wipe the cache on a full scan to force sending all available data
        if force_full_scan {
        ...
        // ==========================================
        // MACRO 1: Handle standalone scalar fields
        // ==========================================
        macro_rules! check_scalar {
            ($task_id:ident, $group:ident, $group_type:ident, $field:ident, $getter:expr) => {
                let should_run = force_full_scan || task_map.contains_key(&$task_id);
                if should_run {
                    let limit = if force_full_scan { None } else { task_map.get(&$task_id).and_then(|t| t.limit.clone()) };
                    let last = self.last_info_payload.$group.get_or_insert_with($group_type::default);
                    // The trait naturally unwraps Option<T>, so we can pass getters directly!
        ...
        check_scalar!(BatteryPercentage, battery, BatteryPayload, power_plugged, self.info.get_battery_is_plugged_in());


        // ==========================================
        // MACRO 2: Handle Lists inside Groups (e.g. Disks, Ram)
        // ==========================================
        ...
        // ==========================================
        // MACRO 3: Handle Flat Lists (e.g. GPUs, Networks)
        // ==========================================
        ...
        check_nested_array! {
            ...
            extra: (last_disk, diff, has_diff) => {
                let should_run = force_full_scan || task_map.contains_key(&DiskPhysicalPartition);
                if should_run {
                    let limit = if force_full_scan { None } else { task_map.get(&DiskPhysicalPartition).and_then(|t| t.limit.clone()) };
        ...
        let run_process_count = force_full_scan || task_map.contains_key(&ProcessCount);
        if run_process_count {
            let limit = if force_full_scan { None } else { task_map.get(&ProcessCount).and_then(|t| t.limit.clone()) };
        ...
        let run_software = force_full_scan || task_map.contains_key(&Software);
        if run_software {
            info_payload.software = self.info.get_software_list();
        }

        //: Make sure it has ID, either machine serial or motherboard serial
        ...
        machine.serial = match self.info.get_product_serial() {
            Some(serial) => Some(AgentValue::Text(serial.to_string())),
            None => match self.info.get_motherboard_serial() {
                Some(mobo_serial) => {
...
// ✨ THE TRICK: Automatically handle Option wrappers
...
// ✨ THE TRICK: Native boolean handling without casting to u64
